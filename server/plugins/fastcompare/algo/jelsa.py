...
class EasyStudyELSA(AlgorithmBase, ABC):
    def __init__(
        ...
        **kwargs,
        ...
        self._remove_top_n_items = remove_top_n_items
    ...
    def predict(self, selected_items, filter_out_items, k):
        ...
        remove_top_n_items = self._remove_top_n_items
        random_pool_size = self._random_pool_size

        # Get similar items
        # Post processing: Select a larger pool of items to sample
        item_ids, scores = self.model_wrapper.similar_items(
            N=random_pool_size + remove_top_n_items + k + len(filter_out_items),
            batch_size=128,
        ...
        # Post processing: Remove top 200 items, to avoid popularity bias
        if self._verbose:
            # print("Removing top 200 items to avoid popularity bias.")
            print(f"Removing top {remove_top_n_items} items to avoid popularity bias.")
        filtered_items_with_scores = filtered_items_with_scores[remove_top_n_items:]

        # Post processing: Shuffle the items to avoid bias
        if self._verbose:
            print("Shuffling items to avoid bias.")
        np.random.shuffle(filtered_items_with_scores)
    ...
    def parameters(cls):
        return [
            ...
            Parameter(
                ...
                ParameterType.INT,
                20,
                help="Number of top items to remove to avoid popularity bias",
                help_key="elsa_remove_top_n_items_help",
            ),
            Parameter(
                "random_pool_size",
                ParameterType.INT,
                200,
                help="Size of the random pool to sample from",
                help_key="elsa_random_pool_size_help",
            ),
            Parameter(
                "verbose",
                ParameterType.BOOL,
